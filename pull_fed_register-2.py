...
STRONG_KW = ("medicare advantage", "ma organization", "d-snp", "c-snp",
             "special needs plan", "medigap", "medicare supplement", "dual eligible")
SKIP_TITLE = [
    ("agency information collection", "PRA paperwork notice"),
    ("privacy act", "Privacy Act records notice"),
    ("quarterly listing", "catalog of issuances"),
    ("deeming authority", "accreditor approval"),
    ("accrediting organization", "accreditor approval"),
    ("amount in controversy", "appeals threshold housekeeping"),
    ("provider enumeration", "admin systems notice"),
    ("public data asset", "data release notice"),
    ("meeting of the", "meeting notice"),
    ("town hall", "meeting notice"),
]
PROVIDER_PAY = ["prospective payment system", "physician fee schedule",
    "outpatient prospective payment", "home health", "hospice wage index",
    "end-stage renal", "inpatient rehabilitation", "psychiatric facilities",
    "skilled nursing", "disproportionate share hospital", "organ transplant access"]


def classify(title, hits):
    """Priority tiers: HIGH/MEDIUM = read, LOW = marginal, SKIP = noise (with reason)."""
    t = (title or "").lower()
    strong = sum(hits.get(k, 0) for k in STRONG_KW)
    for pat, reason in SKIP_TITLE:
        if pat in t:
            return "SKIP", reason
    if any(p in t for p in PROVIDER_PAY) and strong < 200:
        return "SKIP", "provider payment rule (MA mentions incidental)"
    if "actuarial rates, premium rates" in t:
        return "MEDIUM", "annual Part B premium/deductible notice - consumer-visible"
    if strong >= 100:
        return "HIGH", f"core MA/Part D policy doc (strong kw={strong})"
    if strong >= 10:
        return "MEDIUM", f"skim - moderate MA/SNP relevance (strong kw={strong})"
    return "LOW", f"marginal (strong kw={strong})"
...
def main():
    ...
    for i, doc in enumerate(docs, 1):
        ...
        pri, note = classify(doc.get("title", ""), hits) if hits else ("", "")
        rows.append({
            "num": doc["document_number"],
            "type": doc.get("type", ""),
    ...
    for r, row in enumerate(rows, 2):
        vals = [row["num"], row["type"], row["pub"], row["eff"], row["match"],
                ...
                row["priority"], row["note"]]
        for c, v in enumerate(vals, 1):
            cell = ws.cell(row=r, column=c, value=v)
            cell.font = body_font
            if c in (3, 4) and v:
                cell.number_format = "m/d/yyyy"
    widths = [16, 14, 11, 13, 7, 40, 60, 60, 45, 18, 10, 48]
    ...
    n_hi = sum(1 for r in rows if r.get("priority") in ("HIGH", "MEDIUM"))
    print(f"filter Priority to HIGH/MEDIUM ({n_hi} docs) and start reading.")
